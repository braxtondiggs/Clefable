...
// Plugin Constructor
	...
	// If no caching or file is not cached
		...
		// Handle Styles	
			if(extension == 'css') {
				(function(){
					var opts = $.extend({url: url},defaults.ajax),
	...
	// after files have been parsed by browser
		if(this.queue.length == 0 && callback) {
			setTimeout(function(){callback.apply(that); delete that.tmp_callback; },0);
		}
	...
	Plugin.prototype.get = function(){
		var that = this,
			...
			callback = arguments[0] || this.callback;
	
	// Store Load callback
	// This will be called when all files have finised loading
